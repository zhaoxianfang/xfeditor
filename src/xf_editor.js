;(function(factory) {
    ...
    xfEditor.prototype    = xfEditor.fn = {
        ...
        save : function() {
            ...
            var markdownProtected = xfEditor.protectTeXSyntax(cmValue);
            // ★ v1.17.2: 预处理链接 target 语法 [text](url){target=_blank} → [text](url "target=_blank")
            markdownProtected = xfEditor.preprocessLinkTarget(markdownProtected);
            var preprocessResult = xfEditor.preprocessMarkdownBlocks(markdownProtected, rendererOptions);
            var newMarkdownDoc = xfEditor.$marked(preprocessResult.markdown, markedOptions);
            newMarkdownDoc = xfEditor.restorePlaceholders(newMarkdownDoc, preprocessResult.placeholders);
            newMarkdownDoc = xfEditor.restoreTeXSyntax(newMarkdownDoc);
            newMarkdownDoc = xfEditor.fixSmartypantsHTML(newMarkdownDoc);
            newMarkdownDoc = xfEditor.fixTableEmptyCells(newMarkdownDoc);
            if (settings.taskList) newMarkdownDoc = xfEditor.postProcessTaskLists(newMarkdownDoc);

            //console.info("cmValue", cmValue, newMarkdownDoc);

            newMarkdownDoc = xfEditor.filterHTMLTags(newMarkdownDoc, settings.htmlDecode);
        ...
        getPreviewedHTML : function(options) {
            ...
            if (this.previewContainer && this.previewContainer.html() && this.previewContainer.html().trim() !== '') {
                ...
                var markdownProtected3 = xfEditor.protectTeXSyntax(markdownText);
                markdownProtected3 = xfEditor.preprocessLinkTarget(markdownProtected3);
                var mdPreprocess = xfEditor.preprocessMarkdownBlocks(markdownProtected3, rendererOptions);
                rawHTML = xfEditor.$marked(mdPreprocess.markdown, markedOptions);
                rawHTML = xfEditor.restorePlaceholders(rawHTML, mdPreprocess.placeholders);
                rawHTML = xfEditor.restoreTeXSyntax(rawHTML);
                rawHTML = xfEditor.fixSmartypantsHTML(rawHTML);
    ...
    xfEditor.markdownToHTML = function(id, options) {
        ...
        var mdProtected = xfEditor.protectTeXSyntax(markdownDoc);
        mdProtected = xfEditor.preprocessLinkTarget(mdProtected);
        // 在 marked 解析之前预处理自定义块级语法
        var mdPreprocess = xfEditor.preprocessMarkdownBlocks(mdProtected, rendererOptions);
        var markdownParsed = marked(mdPreprocess.markdown, markedOptions);
        markdownParsed = xfEditor.restorePlaceholders(markdownParsed, mdPreprocess.placeholders);
        markdownParsed = xfEditor.restoreTeXSyntax(markdownParsed);
        markdownParsed = xfEditor.fixSmartypantsHTML(markdownParsed);
        markdownParsed = xfEditor.fixTableEmptyCells(markdownParsed);
        if (settings.taskList) markdownParsed = xfEditor.postProcessTaskLists(markdownParsed);

        markdownParsed = xfEditor.filterHTMLTags(markdownParsed, settings.htmlDecode);
    ...
    xfEditor.isIE    = (navigator.appName == "Microsoft Internet Explorer");
    xfEditor.isIE8   = (xfEditor.isIE && navigator.appVersion.match(/8./i) == "8.");
