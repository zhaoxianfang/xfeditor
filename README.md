...
| 特性 | 说明 |
|------|------|
| **同步滚动全面优化** | `_buildHeadingLineMap` 新增缩进代码块检测，排除 display:none/visibility:hidden 的隐藏标题；`_syncPreviewToEditor` 容差从 -8px 提升至 -25px，增加 DOM 元素有效性验证、headingMap 自动重建；新增 charCoords 精确定位回退机制；`_syncEditorToPreview` 增加 DOM attachment 验证 |
...
| **🔄 同步滚动强化** | 新增 rAF 防抖 + 锁机制（`_syncLockScroll`）防止递归触发；activeSide 超时从 3s 延长至 5s 防止惯性滚动提前清除；修复 preview.css 缺失 Tabs/Columns 等组件样式 |
